...
class ThreeApp {
  ...
  #postprocessing: { setSize: (w: number, h: number) => void; dispose: () => void; render: () => void } | undefined;
  ...
  updateWorldSize() {
    if (this.camera.isPerspectiveCamera) {
      ...
      (this.camera as PerspectiveCamera & { isOrthographicCamera?: boolean })
...
const defaultBallpitConfig: BallpitMeshConfig = {
  count: 200,
  colors: [0, 0, 0],
...
class BallpitMesh extends InstancedMesh {
  ...
  constructor(renderer: WebGLRenderer, params: Partial<BallpitMeshConfig> = {}) {
  ...
  update(deltaInfo: { delta: number }) {
    ...
    for (let idx = 0; idx < this.count; idx++) {
      instanceDummy.position.fromArray(this.physics.positionData, 3 * idx);
      if (idx === 0 && this.config.followCursor === false) {
        instanceDummy.scale.setScalar(0);
      } else {
        instanceDummy.scale.setScalar(this.physics.sizeData[idx]);
      }
      instanceDummy.updateMatrix();
      this.setMatrixAt(idx, instanceDummy.matrix);
      if (idx === 0) this.light!.position.copy(instanceDummy.position);
    }
...
function createBallpit(
  canvas: HTMLCanvasElement,
  config: Partial<BallpitMeshConfig> = {},
): CreateBallpitReturn {
  const threeInstance = new ThreeApp({
    canvas,
    size: "parent",
  ...
  function initialize(cfg: Partial<BallpitMeshConfig>) {
    ...
    spheres = new BallpitMesh(threeInstance.renderer, cfg);
